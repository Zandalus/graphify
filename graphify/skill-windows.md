...
```powershell
& (Get-Content {{OUTPUT_DIR}}\.graphify_python) -c "
import json, os
from pathlib import Path
...
" | Out-File -FilePath {{OUTPUT_DIR}}\.graphify_transcripts.json -Encoding utf8
```
...
```powershell
python -m graphify.serve {{OUTPUT_DIR}}/graph.json
```
...
Remove-Item -ErrorAction SilentlyContinue {{OUTPUT_DIR}}/.needs_update
```
...
Before the merge step, save the old graph: `Copy-Item {{OUTPUT_DIR}}/graph.json .graphify_old.json`
