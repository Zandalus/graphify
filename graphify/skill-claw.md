...
cached_nodes, cached_edges, cached_hyperedges, uncached = check_semantic_cache(all_files, output_dir='{{OUTPUT_DIR}}')
...
saved = save_semantic_cache(new.get('nodes', []), new.get('edges', []), new.get('hyperedges', []), output_dir='{{OUTPUT_DIR}}')
...
n = to_obsidian(G, communities, '{{OUTPUT_DIR}}/obsidian', community_labels=labels or None, cohesion=cohesion)
print(f'Obsidian vault: {n} notes in {{OUTPUT_DIR}}/obsidian/')

to_canvas(G, communities, '{{OUTPUT_DIR}}/obsidian/graph.canvas', community_labels=labels or None)
print('Canvas: {{OUTPUT_DIR}}/obsidian/graph.canvas - open in Obsidian for structured community layout')
print()
print('Open {{OUTPUT_DIR}}/obsidian/ as a vault in Obsidian.')
...
```json
{
  "mcpServers": {
    "graphify": {
      "command": "python3",
      "args": ["-m", "graphify.serve", "/absolute/path/to/{{OUTPUT_DIR}}/graph.json"]
    }
  }
}
```
...
Before the merge step, save the old graph: `cp {{OUTPUT_DIR}}/graph.json .graphify_old.json`
...
Skip Steps 1–3. Load the existing graph from `{{OUTPUT_DIR}}/graph.json` and re-run clustering:
...
Load `{{OUTPUT_DIR}}/graph.json`, then:
...
- **Docs, papers, or images:** writes a `{{OUTPUT_DIR}}/needs_update` flag and prints a notification to run `/graphify --update` (LLM semantic re-extraction required).
